...
function AuthenticatedApp() {
  ...
  return (
    <div className="authenticated-app flex">
      {showSidebar && <Sidebar />}
      <main className={`main-content flex-1 p-8 ${showSidebar ? 'ml-64' : ''}`}>
        <Routes>
          <Route
            path="/dashboard"
            element={
          ...
          <Route path="/UserHome" element={<UserHome />} /> {/* Add route for UserHome */}
          <Route path="/vue-more/:id" element={<VueMore />} /> {/* Add this route */}
          <Route path="/portfolio" element={<VuePortfolio />} /> {/* Add this route */}
          <Route path="/read-jobs" element={<ReadJobs />} /> {/* Add route for ReadJobs */}
        </Routes>
      </main>
    </div>
  );
